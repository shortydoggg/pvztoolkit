...
set pvzv=PvZ_Toolkit_v1.20.5
set pvzdv=PvZ_Toolkitd_v1.20.5
...
mkdir .\out
if not exist .\out\splash.png copy .\misc\splash.png .\out
if not exist .\out\lineup_(English).yml copy .\misc\lineup_(English).yml .\out
if not exist .\out\builds_(English).yml copy .\misc\builds_(English).yml .\out
if not exist .\out\*.reg copy .\misc\*.reg .\out
...
REM NOTE: Building clear_registry.reg is not needed, since the file is already included in the misc folder.

REM echo Windows Registry Editor Version 5.00>dist\clear_registry.reg
REM echo(>>dist\clear_registry.reg
REM echo [-HKEY_CURRENT_USER\SOFTWARE\Cube Studio\PvZ Toolkit\v1]>>dist\clear_registry.reg
