...
bool PvZ::GameOn()
{
    ...
    {
#ifdef _DEBUG
        std::wcout << L"The game is already open and can be modified." << std::endl;
#endif
    }
    else
    {
        on = FindPvZ();
#ifdef _DEBUG
        std::wcout << L"The game does not open normally, search again: " << (on ? L" found" : L" not found") << std::endl;
#endif
    }
...
void PvZ::AutoLadder(bool imitater_pumpkin_only = true)
{
    ...
    for (size_t i = 0; i < plant_count_max; i++)
    {
        ...
        if (!plant_dead && !plant_squished && plant_type == 30) // 30 南瓜
        {
            ...
            {
#ifdef _DEBUG
                std::wcout << L"Take the ladder:" << (plant_row + 1) << L" " << (plant_col + 1) << std::endl;
#endif
