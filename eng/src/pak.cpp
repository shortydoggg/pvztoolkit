...
{
#ifdef _DEBUG
    std::wcout << L"Unpack source files:" << src_file << std::endl;
    std::wcout << L"Unpack the target folder:" << dst_dir << std::endl;
#endif
...
#ifdef _DEBUG
    std::wcout << L"源文件大小: " << size << "byte" << std::endl;
#endif
...
{
#ifdef _DEBUG
    std::wcout << L"Package source folder:" << src_dir << std::endl;
    std::wcout << L"Package object file:" << dst_file << std::endl;
#endif
