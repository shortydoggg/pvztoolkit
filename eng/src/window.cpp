                           ...
                           int X = 0, int Y = 0, int W = 0, int H = 0)
{
    ...
    case CONTEXT_COL_HEADER:
        if (total == 0) // 僵尸List表为空时不画波数表头
            break;

        if (COL == (20 + 1 - 1)) // 最后一List改为显示总数
    ...
    : Fl_Double_Window(width, height, title)
{
    ...
    int x = m + m;           // No.一个控件的 x 坐标
    int y = m + th + tb + m; // No.一个控件的 y 坐标

    // (4 List的情况下) 根据List数返回 x 坐标
    auto c = [=](int col) -> int
    {
        return x + (iw + m) * (col - 1);
    };

    // (6 OK的情况下) 根据OK数返回 y 坐标
    auto r = [=](int row) -> int
    {
        return y + (ih + m) * (row - 1);
    };
    ...
    // GigaGargantuar weight range in non-huge waves
        ...
        if (hf != INVALID_HANDLE_VALUE)
        {
            do
            {
                ...
                if (ext == L"yml")
                {
                    ...
                    // std::wcout << L"导入阵型List表: " << name << std::endl;
                }
...
void Window::cb_tooltips()
{
    ...
    choice_giga_weight->copy_tooltip(on ? "GigaGargantuar\'s weight in non-huge waves.\nOriginal: 1000 (non-huge waves), 6000 (huge waves)."
                                        : "GigaGargantuar\'s weight in non-huge waves.\nOriginal: 1000 (non-huge waves), 6000 (huge waves).");
    check_giga_limit->copy_tooltip(on ? "No GigaGargantuar in 11th ~ 19th waves."
                                      : "No GigaGargantuar in 11th ~ 19th waves.");
