...
int main(int argc, char **argv)
{
    ...
    {
        fl_message_title("Beta Expiration Reminder");
        if (fl_choice("This is a beta version a long time ago, do you want to download the new official version now?", //
                      "No", "Yes", 0) == 1)
    ...
    if (SIGNATURE_CHECK)
    {
        ...
        if (!VerifySignature(exePath))
        {
            ...
            if (fl_choice("This program may have been infected with a virus, please re-download it from the official channel!", //
                          "No", "Yes", 0) == 1)
                ShellExecuteW(nullptr, L"open", L"https://pvz.lmintlcx.com/toolkit/", //
                              nullptr, nullptr, SW_SHOWNORMAL);
            return -2;
...
#ifdef _DEBUG
    std::wcout << L"Start time (ms):" << (clock() - start) << std::endl;
